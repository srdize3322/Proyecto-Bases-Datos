...
// listo para limpiar el fonasa con la misma paciencia que el otro script
...
// creo las carpetas como siempre
...
$seenCod = [];
$line = 1;
// leer cada fila para depurar
while (($row = fgetcsv($in, 0, $sep, $enc, $esc)) !== false) {
    $line++;
    $raw = $row;
    // guardo la fila original por si toca mandarla a eliminados
    $trimRow($row);
    if (isset($row[$IDX_COD])) {
        $row[$IDX_COD] = $stripBom($row[$IDX_COD]);
    }
    // si no lo mando a errores
    ...
    // si el codigo se repite se elimina
    ...
    // codA
    // el adicional es opcional asi que lo elimino porque me funciono mejor asi
    ...
    // guardo el registro limpio y escribo
    if ($notes) {
