...
// mantengo utf igual que el resto de capas
...
// rutas organizadas igual que los otros filtros

@mkdir("$BASE/Depurado",0777,true);
@mkdir("$BASE/Eliminado",0777,true);
@mkdir("$BASE/Logs",0777,true);
// preparo carpetas para los outputs

// valido que exista la fuente
if (!is_readable($IN)) {
...
$fixMojibake = function ($s) {
    // tabla para limpiar mojibake que ya vimos en fuentes viejas
    $map = [
...
$normRUN = function ($raw, &$why=null) use ($dv11, $onlyDigits) {
    // normalizo run corrigiendo dv cuando falla
...
$normFecha = function ($f) {
    // fecha termina en iso para cruzar con otras tablas
    $f = trim((string)$f);
    if ($f === '') return null;
...
// cargo los run aceptados antes de leer atenciones
$runValidos = [];
...
// --- Preparar IO ---
// archivos listos para separar ok y err
$in  = fopen($IN, 'r');
$ok  = fopen($OK, 'w');
$er  = fopen($ERR, 'w');
$lg  = fopen($LOG, 'w');
...
// limpio encabezados para evitar espacios raros
foreach ($header as &$c) {
    $c = $stripBom($c);
}
unset($c);
...
$IDX_DIAG  = 4;
$IDX_EFECT = 5;
// indices fijos segun layout entregado

$seenID = [];
$line = 1;

// recorro cada fila depurando o descartando
while (($row = fgetcsv($in, 0, $sep, $enc, $esc)) !== false) {
    $line++;
    $raw = $row;
    // guardo original por trazabilidad en err
    $trimRow($row);
    if (isset($row[$IDX_ID])) {
        $row[$IDX_ID] = $stripBom($row[$IDX_ID]);
    }

    // descarto filas vacias de inmediato
    ...
    // Fecha
    // transformo la fecha al iso acordado
    ...
    // RUN paciente
    // ajusto run paciente contra tabla personas
    $why = null;
    ...
    // RUN médico
    // repito ajuste para el medico responsable
    $why = null;
    ...
    // Efectuada
    // normalizo el flag a true false esperado
    ...
    // Diagnóstico
    // limpio diagnostico dependiendo del flag efectuada
    ...
    // guardo fila depurada y registro ajustes
...
// cierro archivos para mantener orden
fclose($in); fclose($ok); fclose($er); fclose($lg);
