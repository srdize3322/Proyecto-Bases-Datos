#!/usr/bin/env php
<?php
// depura Orden.csv con FK válidas hacia Atencion y Arancel DCColita.

mb_internal_encoding('UTF-8');
// mantengo utf8 para no perder tildes en descripciones
...
// rutas sincronizadas con los maestros ya depurados
...
// aborta pronto si el insumo base no existe
...
// helpers uniforman cadenas y claves comparables entre fuentes

// --- Precarga catálogos ---
// cargo ids validos de atencion para validar fk
$atencionValid = [];
...
$mapDesc = [];
$ambiguous = [];
// armo referencias para codigo y descripcion normalizada de arancel
...
// archivos de trabajo listos para volcar ordenes depuradas
$in  = fopen($IN, 'r');
$ok  = fopen($OK, 'w');
$err = fopen($ERR, 'w');
$log = fopen($LOG, 'w');
...
fputcsv($ok, $header, ';', '"', '\\');
fputcsv($err, $header, ';', '"', '\\');
// ambos outputs comparten encabezado consistente

$line = 1;
$okCount = 0;
$errCount = 0;
$reasonCount = [];
// contadores permiten resumir la ejecucion al final
...
while (($row = fgetcsv($in, 0, ';', '"', '\\')) !== false) {
    ...
    $notes = [];
    // notas guardan cada ajuste aplicado a la fila
    ...
    // descripcion se compacta para buscar coincidencias de texto
    ...
    // siempre escribo trio [atencion, arancel, descripcion] aunque falten ids

    $reason = null;
    // priorizo validar primero la fk hacia atencion y luego el arancel
    if ($idA === null) {
        ...
        if ($key !== '' && isset($mapDesc[$key])) {
            ...
            $idB = $mapped;
            $out[1] = (string)$mapped;
            // se corrige la fk con la descripcion depurada
        } else {
...
if ($reasonCount) {
    // ordeno principales causas para revision rapida
    arsort($reasonCount);
