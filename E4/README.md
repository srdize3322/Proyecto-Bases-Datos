...
- `SPorden.sql` define `generar_documentos(id_atencion, ruta_base)`, que arma las recetas normales (`recetas/`), psicotrópicas (`recetas_psico/`) y la orden (`ordenes/`) usando únicamente datos existentes de la atención. Pasa la ruta absoluta del proyecto como segundo parámetro (en mi caso especifico es ...) `SELECT generar_documentos(4879, '/Users/santiago/Documentos/Bases de Datos/Entregas/E4/');`).
